...
public class ItemBoxManager : MonoBehaviour
{
    ...
    public GameObject Generate(int index, Vector3 position)
    {
        ...
        GameObject go = SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().ItemBoxCacheSystem.Archive(filePath, position);       
    ...
    public void Prepare()
    {
        ...
        for (int i = 0; i < ItemBoxFiles.Length; i++)
        {
            ...
            SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().ItemBoxCacheSystem.GenerateCache(ItemBoxFiles[i].filePath, go, ItemBoxFiles[i].cacheCount);
        }
    }
    ...
    public bool Remove(ItemBox item)
    {
        ...
        SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().ItemBoxCacheSystem.Restore(item.FilePath, item.gameObject);
        return true;
    }
}
