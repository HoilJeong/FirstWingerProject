...
public class EffectManager : MonoBehaviour
{
    ...
    public GameObject GenerateEffect(int index, Vector3 position)
    {
        ...
        GameObject go = SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().EffectCacheSystem.Archive(filePath, position);      
    ...
    public void Prepare()
    {
        for (int i = 0; i < effectFiles.Length;  i++)
        {
            ...
            SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().EffectCacheSystem.GenerateCache(effectFiles[i].filePath, go, effectFiles[i].cacheCount, this.transform);
        }
    }

    public bool RemoveEffect(AutoCachableEffect effect)
    {
        SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().EffectCacheSystem.Restore(effect.FilePath, effect.gameObject);
        return true;
    }
}
