...
public class Boss : Enemy
{
    ...
    const float FireTransformRatationIntarval = 15.0f; // 발사방향 회전시 간격
    ...
    [SyncVar]
    [SerializeField]
    Vector3 CurrentFireTransformratation;
    ...
    protected override void SetBattleState()
    {
        ...
        // 회전값을 초기화
        CurrentFireTransformratation.z = FireTransformRotationStart;
        Quaternion quat = Quaternion.identity;
        quat.eulerAngles = CurrentFireTransformratation;
    ...
    void SetBattleMove()
    {
        ...
        // 랜덤한 거리를 이동하기 위한 부분
        float newBattleMoveLength = Random.Range(BattleMoveMax, BattleMoveMax * 3.0f); // BattleMoveMax 의 1배 ~ 3배의 사이의 거리를 이동

        RpcSetBattleMove(halfPingpongHeight, newBattleMoveLength); // Host 플레이어인 경우 RPC로 보낸다
    }
    ...
    void UpdateBattleMove()
    {
        ...
        {
            if (isServer)
                RpcChangeBattleMoveTarget(); // Host 플레이어인 경우 RPC로 보낸다
        }
    ...
    {
        if (isServer)
            RpcSetBattleFire(); // Host 플레이어인 경우 RPC로 보낸다
    }
    ...
    public void RpcSetBattleFire()
    {
        ...
        quat.eulerAngles = CurrentFireTransformratation;
        fireTransform.localRotation = quat;
        SpecialAttack = !SpecialAttack; // 일반 공격과 미사일 공격을 번갈아 가면서 사용
    ...
    {
        if (isServer)
            RpcRotateFireTransform(); // Host 플레이어인 경우 RPC로 보낸다
    }

    [ClientRpc]
    public void RpcRotateFireTransform()
    {
        CurrentFireTransformratation.z += FireTransformRatationIntarval;
        Quaternion quat = Quaternion.identity;
        quat.eulerAngles = CurrentFireTransformratation;
        fireTransform.localRotation = quat;
    ...
    public void FireChase()
    {
        ...
        GuidedMissile missile = SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().BulletManager.Genelate(BulletManager.GuidedMissileIndex, missileFireTransform.position) as GuidedMissile;
        if (missile)
        {          
    ...
    protected override void OnDead()
    {
        ...
        if (isServer)
            SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().OnGameEnd(true);
    }
}
