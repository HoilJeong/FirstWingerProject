...
public class DamageManager : MonoBehaviour
{
    ...
    public void Prepare()
    {
        for (int i = 0; i < Files.Length; i++)
        {
            ...
            SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().DamageCacheSystem.GenerateCache(Files[i].filePath, go, Files[i].cacheCount, canvasTransform);
        }
    }
    ...
    public GameObject Generate(int index, Vector3 position, int damageValue, Color textColor)
    {
        ...
        GameObject go = SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().DamageCacheSystem.Archive(filePath, Camera.main.WorldToScreenPoint(position));  
    ...
    {
        SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().DamageCacheSystem.Restore(damage.FilePath, damage.gameObject);
        return true;
    }
}
