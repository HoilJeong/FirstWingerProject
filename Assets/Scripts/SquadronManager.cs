...
public class SquadronManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (!ShowWarningUICalled)
        {
            ...
            if (SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().EnemyManager.GetEnemyListCount() == 0)
            {
                firstWingerSceneMain.ShowWarningUI();
    ...
    void GenerateSquadron(SquadronTable table)
    {
        ...
        for (int i = 0; i < table.GetCount(); i++)
        {
            ...
            SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().EnemyManager.GenerateEnemy(squadronMember);
        }
    }
