...
public class GuidedMissile : Bullet
{
    ...
    public void FireChase(int targetInstanceID, int ownerInstanceID, Vector3 direction, float speed, int damage)
    {
        ...
        RpcSetTargetInstanceID(targetInstanceID); // Host 플레이어인 경우 RPC
    ...
    protected override void UpdateMove()
    {
        ...
        {
            Actor target = SystemManager.Instance.GetCurrentSceneMain<FirstWingerSceneMain>().ActorManager.GetActor(TargetInstanceID);
            if (target != null)
            {
                // 현재 위치에서 타겟까지 벡터
